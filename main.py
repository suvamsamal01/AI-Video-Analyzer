...
class Worker(QThread):
    ...
    # ---------------------------------------------------------------- #
    #  JSON cleaner — handles qwen3/mistral/llama markdown output       #
    # ---------------------------------------------------------------- #
    def _clean_json(self, raw):
        """
        Robustly extract JSON from model output that may contain:
        - <think>...</think> blocks  (qwen3)
        - ```json ... ```  code fences
        - Markdown headings / bold text before/after JSON
        ...
        text = text.strip()
        text = re.sub(r'^[^{\[]+', '', text)  # leading prose hata do
...
TRANSCRIPT: {trans_short}"""}
                ...
                # ---- FIX: Try array first, then object wrapper ----
                parsed = None
                ...
                # Attempt 2: find [ ... ] array explicitly if parse failed
                if parsed is None:
                ...
                # Unwrap if model returned {"questions": [...]} style
        ...
        def get_chapters():
            try:
                ...
                try:
                    ...
                    chapters = self._parse_chapters_markdown(r)
                ...
                for i, ch in enumerate(chapters):
                    ...
                    fixed.append({
                        ...
                        "start": start_str,
                        "summary": ch.get("summary", ch.get("description", "No summary.")),
                        "description": ch.get("description", ch.get("summary", "No description.")),
                    })
                    for ch in fixed:
                     if not ch["summary"] or ch["summary"].strip() in ["", "No summary.", "No description."]:
                        ch["summary"] = "This chapter covers content from the video. See transcript for details."
                    if not ch["description"] or ch["description"].strip() in ["", "No summary.", "No description."]:
                        ch["description"] = ch["summary"]
                ...
                results["chapters"] = fixed if fixed else _default_chapters()
